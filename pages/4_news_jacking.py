...
def _render_newsjack_ideas(ideas_raw: str, story_context: str, key_prefix: str):
    ...
    for idx, (title, body) in enumerate(parsed):
        ...
        with build_col:
            if st.button(
                f"Build PR Pack around Idea {idx + 1} →",
...
st.set_page_config(page_title="News-Jacking | Riot PR Desk", page_icon=get_page_icon(), layout="wide")
apply_global_styles()
render_sidebar()
...
if not ai_configured():
    st.error("**AI engine not configured.** Add your API key to `.env` to generate news-jack ideas.")
    st.stop()
                    ...
                    if st.button("Suggest News-Jack", key=f"nj_{i}", use_container_width=True, type="primary"):
                        ...
                        with st.spinner("Thinking of creative ways to hijack this story..."):
                            try:
                                ...
                                st.error(f"Failed: {e}")
                with col_pr:
                    if st.button("Create PR Pack →", key=f"nj_gen_{i}", use_container_width=True):
    ...
    if not upcoming:
        ...
        for j, event in enumerate(upcoming):
            ...
            with st.expander(
                ...
                with col_b:
                    if is_custom:
                        if st.button("Remove", key=f"cal_del_{j}", use_container_width=True):
                ...
                if f"cal_result_{j}" in st.session_state:
                    ...
                    if cal_vote_key in st.session_state:
                        st.caption(f"{'Good' if st.session_state[cal_vote_key] == 'up' else 'Weak'} — Vote recorded")
                    else:
                        ...
                        with cv1:
                            if st.button("Good", key=f"cal_up_{j}"):
